...
\begin{frame}
    ...
    \begin{center}
    \begin{tikzpicture}
    \node[draw, rounded corners, fill=blue!10, text width=3cm] (family) at (0,0) {Family:\\Simpsons, Flanders\\(Immediate unity)};
    \node[draw, rounded corners, fill=green!10, text width=3cm] (civil) at (4,0) {Civil Society:\\Power Plant, Kwik-E-Mart\\(Particular interests)};
    \node[draw, rounded corners, fill=red!10, text width=3cm] (state) at (8,0) {State:\\Mayor Quimby, Town Hall\\(Universal interest)};
    
    \draw[->, thick] (family) -- (civil);
    \draw[->, thick] (civil) -- (state);
    \draw[->, thick, bend right=30] (family) to node[below] {Dialectical development} (state);
    \end{tikzpicture}
    \end{center}
    \end{frame}
