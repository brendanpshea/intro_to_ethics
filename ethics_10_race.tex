...
\begin{document}
	...
	\begin{frame}{Civil Rights Movement: Philosophical Underpinnings}
		...
		\begin{tikzpicture}
			...
			\node[above] at (3,0.5) {Sit-ins};
			\node[above] at (5,0.5) {Voting Rights Act};
			\node[above] at (7.5,0.5) {King Assassination};
		\end{tikzpicture}
	\end{frame}
	...
	\begin{frame}{Du Bois on Education: The Talented Tenth and Beyond}
		...
		\begin{tikzpicture}[]
			\draw [thick, ->] (0,0) -- (0,4);
			\draw [thick, ->] (0,0) -- (8,0);
			\node [left] at (0,4) {Educational Focus};
			\node [below] at (8,0) {Time};
			\node [below] at (3,0) {1903};
			\node [below] at (6,0) {1948};
			\draw [thick, blue] (0,3) -- (3,3) -- (6,1);
			\node [above] at (1.5,3) {Talented Tenth};
			\node [below] at (4.5,1) {"Democratic Education"};
		\end{tikzpicture}
	\end{frame}
	...
	\begin{frame}{King's Critique of Moderate Liberalism}
		\begin{itemize}
			\item King identified the "white moderate" as a greater obstacle to justice than the explicit racist due to preference for order over justice.
	...
	\begin{frame}{Beyond Race: King's Later Philosophy on Poverty and War}
		\begin{itemize}
			...
			\item King's opposition to the Vietnam War connected militarism, racism, and materialism as "triple evils" threatening human flourishing.
	...
	\begin{frame}{The Ethics of Identity: Philosophical Framework}
		...
		\begin{center}
			\begin{tikzpicture}
				\draw [<->] (0,0) -- (8,0);
				\node [below] at (0,0) {Strong Collectivism};
				\node [below] at (8,0) {Radical Individualism};
				\node [below, text width=3cm, align=center] at (4,0) {Appiah's \\ Ethical Balance};
				\filldraw (4,0) circle (4pt);
				\node [above] at (2,0.5) {Identity Scripts};
				\node [above] at (6,0.5) {Personal Autonomy};
			\end{tikzpicture}
		\end{center}
	\end{frame}
